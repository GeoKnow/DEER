...
\subsection{RDF Configuration main nodes}
\geolift RDF configuration contains four main resource types (dataset, module, operator and parameter nodes).
...
\subsubsection{Dataset Resource}
    ...
    Beside defining a resource as a dataset (ex: figure~\ref{fig:rdfconf}(lines 4),
    a dataset resource may also contains the following predicates:
    \begin{itemize}
	\item \texttt{rdfs:label} pacifying label for the dataset.
	\item \texttt{:hasUri} setting URI of the dataset resource, to read resource through content negotiation (ex: figure~\ref{fig:rdfconf}(lines 5).
	\item \texttt{:FromEndPoint} end point to read the CBD from. 
	\emph{Note: must be used in conjunction with \texttt{:hasUri} predicate, otherwise an error will be generated by \geolift.} (ex: figure~\ref{fig:rdfconf}(lines 6)
	\item \texttt{:inputFile} input file to load the dataset from.
	\item \texttt{:outputFile} output file to save the dataset to (ex: figure~\ref{fig:rdfconf}(lines 14).
	\item \texttt{:outputFormat} output format to save the dataset in it (ex: figure~\ref{fig:rdfconf}(lines 14), Turtle format will be used as default format if this property is not set.
    \end{itemize}

\subsubsection{Module Resource}
    Beside defining a resource as a Module (ex: figure~\ref{fig:rdfconf}(lines 16),
    a Module resource may also contains the following predicates:
    \begin{itemize}
	\item \texttt{rdfs:label} pacifying label for the module (ex: figure~\ref{fig:rdfconf}(lines 17).
	\item \texttt{:hasInput} pacifying input datasets to the module (ex: figure~\ref{fig:rdfconf}(lines 18).
	\item \texttt{:hasOutput} pacifying output datasets to the module (ex: figure~\ref{fig:rdfconf}(lines 19).
	\item \texttt{:hasParameter} pacifying Parameters for the module (ex: figure~\ref{fig:rdfconf}(lines 20).
    \end{itemize}

\subsubsection{Operator Resource}
    Beside defining a resource as an operator (ex: figure~\ref{fig:rdfconf}(lines 24),
    a Module resource may also contains the following predicates:
    \begin{itemize}
	\item \texttt{rdfs:label} pacifying label for the operator (ex: figure~\ref{fig:rdfconf}(lines 25).
	\item \texttt{:hasInput} pacifying input datasets to the operator (ex: figure~\ref{fig:rdfconf}(lines 26).
	\item \texttt{:hasOutput} pacifying output datasets to the operator (ex: figure~\ref{fig:rdfconf}(lines 27).
	\item \texttt{:hasParameter} pacifying Parameters for the operator.
    \end{itemize}

\subsubsection{Parameter Resource}
    Beside defining a resource as a parameter (ex: figure~\ref{fig:rdfconf}(lines 21),
    a Module resource may also contains the following predicates:
    \begin{itemize}
	\item \texttt{rdfs:label} pacifying label for the parameter.
	\item \texttt{:hasKey} pacifying the parameter key (ex: figure~\ref{fig:rdfconf}(lines 22).
	\item \texttt{:HasValue} pacifying the parameter value (ex: figure~\ref{fig:rdfconf}(lines 23).
    \end{itemize}
