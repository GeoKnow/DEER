...
public class DereferencingModule implements DeerModule {
  ...
  private static final String RESOURCE_PREFIX_Desc =
    "Resource prefix used for dereferencing resources, by default is set to "
  ...
  private void readParameters(Map<String, String> parameters) {
    for (String key : parameters.keySet()) {
      ...
      if (key.equalsIgnoreCase("usecache")) {
        ...
        logger.error("Invalid parameter key: " + key
          + ", allowed parameters for the dereferencing module are: " + getParameters());
    ...
    if (inputProperties.size() == 0) {
      logger.error(
        "The " + INPUT_PROPERTY + " is a mandatory parameter(s) for the dereferencing module");
...
//		ignoreProperties.add(OWL.sameAs);
   ...
   * @param parameters : list of predicates used to enrich information in model returns model after
   * enrichment
