...
public class FusionOperator implements IOperator {
  ...
  private static final String DBPEDIA_ONTOLOGY_201504 = "src/main/resources/org.aksw.deer.resources.fusion/dbpedia_201504.owl";
  private static final String DBPEDIA_ONTOLOGY_3_9 = "src/main/resources/org.aksw.deer.resources.fusion/dbpedia_3.9.owl";
  private static final String DBPEDIA_ONTOLOGY = "src/main/resources/org.aksw.deer.resources.fusion/dbpedia.owl";
  ...
  public static OWLClassExpression learnOWLClassExpression(Set<OWLIndividual> posExamples,
    Set<OWLIndividual> negExamples, String endPoint) throws ComponentInitException {
    logger.info("starting learning task ...");
    logger.info("initializing knowledge source...");
    Model model = ModelFactory.createDefaultModel();
    OWLOntology ontology = getOWLOntology((new ModelReader()).readModel(DBPEDIA_ONTOLOGY));
...
//		test4AllLangTags(args[0]);
  ...
  public static Map<OWLIndividual, Map<String, Double>> getBestOfAllLangTags(String trainingFile,
    String testingFile) throws IOException, ComponentInitException {
    int exCnt = 10;
...
//			OWLClassExpression BestClsExp = bestDesc.getDescription();
  ...
  public static Map<String, OWLClassExpression> _learnFromExamples(String inputFile)
    throws IOException, ComponentInitException {
    Examples examples = new Examples(inputFile);
    for (String langTag : examples.getAvailableLanguageTags()) {
      if (getEndPoint(langTag) != null) {
        int ExampleCount = 10;
        OWLClassExpression bestExpr = learnOWLClassExpression(
          examples.getPositiveExamples("en", ExampleCount),
          examples.getNegativeExamples("en", ExampleCount), getEndPoint(langTag));
  ...
  public static void test() {
    URL path = FusionOperator.class.getClassLoader().getResource("org.aksw.deer.resources.fusion/");
  ...
  public List<Model> process(final List<Model> models, final Map<String, String> parameters) {
    ...
    if (parameters != null) {
      for (String key : parameters.keySet()) {
        if (key.toLowerCase().startsWith(FUNCTIONAL_PROPERTY)) {
          ...
          posExMdl = (new ModelReader()).readModel((parameters.get(key)));
    ...
    for (Property fp : funcProp) {
      for (int i = 0; i < posTriplesMdls.size(); i++) {
        ...
        StmtIterator listStatements = posTriplesMdls.get(i)
          .listStatements(null, fp, (RDFNode) null);
