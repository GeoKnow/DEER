...
public class PRFComputer {
   ...
   * @author sherif
   */
  public synchronized static double getOverlap(final Collection<OWLIndividual> m,
    final Collection<OWLIndividual> reference) {
    double counter = 0;
   ...
   * @author sherif
   */
  public static double computePrecision(final Collection<OWLIndividual> m,
    final Collection<OWLIndividual> reference) {
    if (m.size() <= 0) {
      return 0d;
    }
   ...
   * @author sherif
   */
  public static double computeRecall(final Collection<OWLIndividual> m,
    final Collection<OWLIndividual> reference) {
   ...
   * @author sherif
   */
  public static double computeFScore(final Collection<OWLIndividual> m,
    final Collection<OWLIndividual> reference) {
    if (m.size() == 0) {
      return 0d;
    }
    double overlap = getOverlap(m, reference);
    return 2 * (overlap / (double) m.size()) * (overlap / (double) reference.size()) / (
      overlap / (double) m.size() + (overlap / (double) reference.size()));
  }
}
