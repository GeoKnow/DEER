...
public class FusionOperator implements DeerOperator {
  ...
  public static OWLClassExpression learnOWLClassExpression(Set<OWLIndividual> posExamples,
    ...
    OWLOntology ontology = getOWLOntology((new Reader()).readModel(DBPEDIA_ONTOLOGY));
...
//			OWLClassExpression BestClsExp = bestDesc.getDescription();
  ...
  public static void test() {
    ...
    for (int i = 1; i <= 3; i++) {
      testMdls.add((new Reader()).readModel(path + "s" + i + ".nt"));
    }
    ...
    if (parameters != null) {
      for (String key : parameters.keySet()) {
        if (key.toLowerCase().startsWith(FUNCTIONAL_PROPERTY)) {
          ...
          posExMdl = (new Reader()).readModel((parameters.get(key)));
