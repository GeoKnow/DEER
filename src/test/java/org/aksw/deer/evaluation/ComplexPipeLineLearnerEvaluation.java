...
import org.aksw.deer.util.FMeasure;
...
public class ComplexPipeLineLearnerEvaluation {
  ...
  private static final Logger logger = Logger
    .getLogger(ComplexPipeLineLearnerEvaluation.class.getName());
  ...
  public static void main(String args[]) throws IOException {
    if (args.length < 6) {
      logger.error(
        "Parameters: kbFile kbSampleFile authority specComplexity Min_MaxSpecSize Min_MaxExampleNr");
      System.exit(1);
    }
    int i = 0;
    String kbFile = args[i];
    i++;
    String kbSampleFile = args[i];
    i++;
    String authority = args[i];
    i++;
    double specComplexity = Double.parseDouble(args[i]);
    i++;
    int minSpecSize = Integer.parseInt(args[i].substring(0, args[i].lastIndexOf("_")));
    int maxSpecSize = Integer
      .parseInt(args[i].substring(args[i].lastIndexOf("_") + 1, args[i].length()));
    i++;
    int minExampleNr = Integer.parseInt(args[i].substring(0, args[i].lastIndexOf("_")));
    int maxExampleNr = Integer
      .parseInt(args[i].substring(args[i].lastIndexOf("_") + 1, args[i].length()));
    i++;
    ...
    for (int exampleNr = minExampleNr; exampleNr <= maxExampleNr; exampleNr++) {
      for (int specsize = minSpecSize; specsize <= maxSpecSize; specsize++) {
        logger.info(
          "=============================================== Testing for specs size " + specsize
            + ", with complexity " + specComplexity + "==========================================");
        ComplexPipeLineLearnerEvaluation e = new ComplexPipeLineLearnerEvaluation();
        resultStr += e
          .testExampleCount(kbFile, kbSampleFile, specsize, specComplexity, authority, exampleNr,
            0.75);
      }
    }
  ...
  public void test(String inputKBFile, int maxSpecSize, double maxSpecComplexity)
    throws IOException {
    Model s = new ModelReader().readModel(inputKBFile);
    ...
    for (float c = 0; c <= maxSpecComplexity; c++) {
      for (int i = 0; i <= maxSpecSize; i++) {
        ...
        resultStr += ConfigAnalyzer.getModules(mSpecs).size() + "\t";
        ...
        resultStr += ConfigAnalyzer.getModules(r.configModel).size() + "\t";
        resultStr += ConfigAnalyzer.getOperators(r.configModel).size() + "\t";
  ...
  public String testExampleCount(
    ...
    Model kbMdl = new ModelReader().readModel(kbInputFile);
    String kbManuallyEnrichedOutputFile =
      kbInputFile.substring(0, kbInputFile.lastIndexOf(".")) + "_manually_enriched.ttl";
    ...
    List<Resource> resources = SimplePipelineLearnerEvaluation
      .getAllResourcesWithAuthority(authority, kbMdl);
    Model cbdMdl;
    ...
    do {
      ...
      //			mSpecs.write(System.out,"TTL");
      manualSpecsMdl = RandomSpecsGenerator
        .generateSpecs(kbInputFile, kbManuallyEnrichedOutputFile, specsSize, specsComplexity);
      ...
      new ModelWriter().writeModel(cbdMdl, "TTL", cbdOutFile);

      // (2) Generate a manual-config file to the generated CBDs in (1) and save it
      manualSpecsMdl = SimplePipelineLearnerEvaluation
        .changeInputFile(manualSpecsMdl, null, cbdOutFile);
      //			manualSpecsMdl.write(System.out,"TTL");
      ...
      manualSpecsMdl = SimplePipelineLearnerEvaluation
        .changeOutputFile(manualSpecsMdl, kbManuallyEnrichedOutputFile, cbdMSpecsOutputFile);
      //			manualSpecsMdl.write(System.out,"TTL");
      String cbdManualSpecOutputFile = folder + "manual_config_" + exCount + "_examples.ttl";
      new ModelWriter().writeModel(manualSpecsMdl, "TTL", cbdManualSpecOutputFile);
    ...
    ComplexPipeLineLearner learner = new ComplexPipeLineLearner(cbdMdl, manuallyEnrichedCBDMdl,
      penaltyWeight);
    ...
    if (kbSampleFile.isEmpty()) {
      kbManualSpecMdl = SimplePipelineLearnerEvaluation
        .changeInputFile(manualSpecsMdl, cbdOutFile, kbInputFile);
    } else {
      kbManualSpecMdl = SimplePipelineLearnerEvaluation
        .changeInputFile(manualSpecsMdl, cbdOutFile, kbSampleFile);
    }
    kbManualSpecMdl = SimplePipelineLearnerEvaluation
      .changeOutputFile(kbManualSpecMdl, cbdMSpecsOutputFile, kbManuallyEnrichedOutputFile);
    ...
    if (kbSampleFile.isEmpty()) {
      kbSelfConfigMdl = SimplePipelineLearnerEvaluation
        .changeInputFile(lSpecs, inputFile, kbInputFile);
    } else {
      kbSelfConfigMdl = SimplePipelineLearnerEvaluation
        .changeInputFile(lSpecs, inputFile, kbSampleFile);
    }
    String outFile = "outputFile.ttl";
    String kbSOutFile =
      kbInputFile.substring(0, kbInputFile.lastIndexOf(".")) + "_self_enrichmed.ttl";
    kbSelfConfigMdl = SimplePipelineLearnerEvaluation
      .changeOutputFile(kbSelfConfigMdl, outFile, kbSOutFile);
    ...
    // add results
    resultStr += ConfigAnalyzer.getModules(manualSpecsMdl).size() + "\t";
    resultStr += ConfigAnalyzer.getOperators(manualSpecsMdl).size() + "\t";
    resultStr += ConfigAnalyzer.getModules(lSpecs).size() + "\t";
    resultStr += ConfigAnalyzer.getOperators(lSpecs).size() + "\t";
    resultStr += f0.P + "\t";
    resultStr += f0.R + "\t";
    resultStr += f0.F + "\t";
    resultStr += exCount + "\t";
