...
public class SimplePipelineLearnerEvaluation {
  ...
  public static RefinementNodeOld run(String args[], boolean isBatch, int max) throws IOException {
    ...
    for (int i = 1; i <= max; i++) {
      ...
      SimplePipeLineLearner.sourceModel = new ModelReader().readModel(folder + "/input.ttl");
      SimplePipeLineLearner.targetModel = new ModelReader().readModel(folder + "/output.ttl");
  ...
  public static Model changeInputFile(final Model configModel, String oldLocation,
    String newLocation) {
  ...
  public static Model changeOutputFile(final Model configModel, String oldLocation,
    String newLocation) {
    Model result = ModelFactory.createDefaultModel();
    result = result.union(configModel);
...
//		testDBpedia();
  ...
  private static void testDrugBank() throws IOException {
    ...
    for (int i = 1; i <= 2; i++) {
      ...
      new ModelWriter().writeModel(e.cbdSelfConfig, "TTL", folder + "s" + i + ".ttl");
    }
//		System.out.println("resultStr");
...
//		System.out.println("resultStr");
  ...
  public List<Resource> getNResources(String authority, Model kb, int n) {
    List<Resource> results = new ArrayList<Resource>();
    String sparqlQueryString =
      "SELECT DISTINCT ?s { ?s ?p ?o. FILTER (STRSTARTS(STR(?s), \'" + authority + "\')) } LIMIT "
        + n;
    QueryFactory.create(sparqlQueryString);
    QueryExecution qexec = QueryExecutionFactory.create(sparqlQueryString, kb);
    ResultSet queryResults = qexec.execSelect();
  ...
  @SuppressWarnings("unused")
  public String testExampleCount(String kbFile, String kbSampleFile, String manualConfigFile,
    String authority, int exampleCount, double penaltyWeight) throws IOException {
    String folder = kbFile.substring(0, kbFile.lastIndexOf("/") + 1);
    Model kb = new ModelReader().readModel(kbFile);
    Model manualConfig = new ModelReader().readModel(manualConfigFile);
    ...
    do {
      ...
      new ModelWriter().writeModel(cbd, "TTL", cbdOutputFile);
    ...
    SimplePipeLineLearner learner = new SimplePipeLineLearner(cbd, manuallyEnrichedCBD,
      penaltyWeight);
    long start = System.currentTimeMillis();
    ...
    new ModelWriter().writeModel(selfConfEnrichedCBD, "TTL", selfConfEnrichedCbdOutputFile);
    cbdSelfConfig = bestSolution.configModel;
    String cbdSelfConfigOutputFile = folder + "s_config" + exampleCount + ".ttl";
    new ModelWriter().writeModel(cbdSelfConfig, "TTL", cbdSelfConfigOutputFile);
/*
